...
// CODE BLOCK BEGIN On_Start FLD_RLD_RRD_Open
// On start handler "FLD_RLD_RRD_Open"
void on_start_FLD_RLD_RRD_Open(void) { try { // on start event
s32 FLD = 0;
s32 RLD =0;
s32 RRD = 0;
s32 interior = 0;
...
app.get_system_var_int32("FrontRight_Door_Status",&RLD);
app.get_system_var_int32("RearLeft_Door_Status",&RRD);
app.get_system_var_int32("InteriorLight_Status",&interior);
if(FLD==1 && FLD==1 && RRD==1 && interior==1){
app.wait(1000,"");
...
// CODE BLOCK END On_Start FLD_RLD_RRD_Open
